...
import items from "../../../../data/anbiya-duaa.json";
...
const STORAGE_KEY = "am_anbiya_duaa_v1";

function clamp(n: number, min: number, max: number) {
  return Math.max(min, Math.min(max, n));
}

export default function AnbiyaDuaaPage() {
  ...
  const onShare = async () => {
    const text = "أدعية الأنبياء - من القرآن الكريم.";
    try {
