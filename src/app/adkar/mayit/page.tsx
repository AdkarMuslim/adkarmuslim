...
import dhikrsData from "../../../../data/adkar-mayyit.json";
...
const STORAGE_KEY = "am_adkar_mayyit_v1";
...
export default function AdkarMayitPage() {
  ...
  const completedTotal = useMemo(
    () => dhikrs.reduce((acc, d) => acc + clamp(doneById[d.id] ?? 0, 0, d.count), 0),
    [dhikrs, doneById]
  );
  ...
  const onShare = async () => {
    const text = "أدعية للميّت - ذكّرني بما حفظت اليوم.";
    try {
    ...
    <main className="mx-auto w-full max-w-3xl px-2 pb-6">
      <section className="glass-panel ring-accent/0 p-5 sm:p-7">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-white sm:text-3xl">أدعية للميت</h1>
            <p className="mt-3 text-sm leading-relaxed text-white/75">
              أدعية للميت مكتوبة كاملة: استغفار ورحمة وذكر شرعي مع عداد يسهّل المراجعة عند الفقد أو بعد الصلاة على
              الميّت.
            </p>
          ...
          {dhikrs.map((d, idx) => {
            ...
            return (
              <motion.article
                ...
                className={["glass-card p-4 sm:p-5", "border-white/10", completed ? "border-accent/30" : ""].join(" ")}
              >
                <div className="flex flex-col gap-4">
                  <div className="min-w-0">
                    ...
                    <div className="mt-2 max-h-[42vh] w-full overflow-auto overscroll-contain rounded-2xl border border-white/10 bg-white/5 p-4">
                      <div className="w-full text-right text-[1.45rem] font-bold leading-[1.75] text-white md:text-[1.9rem] lg:text-[2rem] whitespace-pre-wrap">
                        {d.text}
                      </div>
                    </div>
                  ...
                  <div className="flex flex-col items-stretch gap-2">
                    ...
                    <button
                      ...
                      aria-label="زيادة العدّ لهذا الدعاء"
                    >
                      {done} / {target}
                    </button>
