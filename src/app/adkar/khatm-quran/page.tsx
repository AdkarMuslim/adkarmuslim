...
import dhikrsData from "../../../../data/adkar-khatm-quran.json";
...
const STORAGE_KEY = "am_adkar_khatm_quran_v1";
...
export default function KhatmQuranDuaPage() {
  ...
  const completedTotal = useMemo(() => {
    return dhikrs.reduce(
      (acc, d) => acc + clamp(doneById[d.id] ?? 0, 0, d.count),
      0
    );
  }, [dhikrs, doneById]);
  ...
  const onShare = async () => {
    const text = "دعاء ختم القرآن الكريم.";
    try {
                ...
                <div className="flex flex-col gap-4">
                  <div className="min-w-0">
                    ...
                    <div className="mt-2 max-h-[42vh] w-full overflow-auto overscroll-contain rounded-2xl border border-white/10 bg-white/5 p-4">
                      <div className="w-full text-right whitespace-pre-wrap text-[1.45rem] font-bold leading-[1.75] text-white md:text-[1.9rem] lg:text-[2rem]">
                        {d.text}
                      </div>
                    </div>
                  ...
                  <div className="flex flex-col items-stretch gap-2">
                    ...
                    <div className="h-2 w-full rounded-full border border-white/10 bg-white/5">
                      <div
                        ...
                        style={{
                          width: `${target === 0 ? 0 : Math.round((done / target) * 100)}%`,
                        }}
                      />
                    </div>
                  </div>
